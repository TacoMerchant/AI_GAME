...
var play_x = obj_player.x;
var play_y = obj_player.y;
...
G1 = instance_create(x, y - 25, class_Node);
G1.dir = 1; // Up

G2 = instance_create(x + 25, y, class_Node);
G2.dir = 2; // Right

G3 = instance_create(x, y + 25, class_Node);
G3.dir = 3; // Down

G4 = instance_create(x - 25, y, class_Node);
G4.dir = 4; // Left
...
while (!ds_priority_empty(openList1) || !ds_priority_empty(openList2)
    ...
    if ((abs(G.x - play_x) < 50) && (abs(G.y - play_x) < 50)) {
        if (G.dir == 1)
            y -= 3.5;
        else if (G.dir == 2)
            x += 3.5;
    ...
    // Generate children
child1 = instance_create(G.x, G.y - 25, class_Node);  // Double check coordinates
child1.dir = G.dir;
child2 = instance_create(G.x + 25, G.y, class_Node);
child2.dir = G.dir;
child3 = instance_create(G.x, G.y + 25, class_Node);
child3.dir = G.dir;
child4 = instance_create(G.x - 25, G.y, class_Node);
child4.dir = G.dir;
...
while (!ds_queue_empty(tempClosedList)) {
    ...
    if ((abs(tempNode.x - child1.x) < 5) && (abs(tempNode.y - child1.y) < 5))
        found1 = true;
    if ((abs(tempNode.x - child2.x) < 5) && (abs(tempNode.y - child2.y) < 5))
        found2 = true;
    if ((abs(tempNode.x - child3.x) < 5) && (abs(tempNode.y - child3.y) < 5))
        found3 = true;
    if ((abs(tempNode.x - child4.x) < 5) && (abs(tempNode.y - child4.y) < 5))
        found4 = true;
    /* 
